...
public class WallPatterns : MonoBehaviour
{
    ...
    // Add to WallPatterns.cs
    public static int GetPattern4Bit(Vector2Int pos, HashSet<Vector2Int> walkableTiles)
    {
        int pattern = 0;
        for (int i = 0; i < Directions.Cardinal.Length; i++)
        {
            if (walkableTiles.Contains(pos + Directions.Cardinal[i]))
                pattern |= (1 << (Directions.Cardinal.Length - 1 - i));  // Reversing bit order
        }
        return pattern;
    }

    public static int GetPattern8Bit(Vector2Int pos, HashSet<Vector2Int> walkableTiles)
    {
        int pattern = 0;
        for (int i = 0; i < Directions.Diagonal.Length; i++)
        {
            if (walkableTiles.Contains(pos + Directions.Diagonal[i]))
                pattern |= (1 << (Directions.Diagonal.Length - 1 - i));  // Reversing bit order
        }
        return pattern;
    }
